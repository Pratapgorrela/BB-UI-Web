...
interface HomeHeaderProps {
  /** Items in the cart. The badge is hidden while this is 0 (no cart exists until F13). */
  cartCount?: number;
}

/**
 * Home greeting header: "Hello" + first name, an address selector, and a cart icon.
 * Address selection (F9) and the cart (F13) don't exist yet, so both taps fire a
 * "coming soon" toast — the established stub pattern.
 */
  ...
  return (
    <header className="flex items-start justify-between gap-3 pt-4 md:hidden">
      ...
      <button
        ...
        onClick={() => addToast('Cart is coming soon', 'info')}
